...
for (const width of viewports) {
  for (const pageName of pages) {
    ...
    const result = await page.evaluate(() => ({
      ...
      overflowElements: [...document.querySelectorAll('body *')].filter(el => !el.closest('.table-responsive') && !el.closest('dialog:not([open])')).map(el => ({el, rect: el.getBoundingClientRect()})).filter(x => x.rect.right > innerWidth + 1 || x.rect.left < -1).slice(0, 12).map(x => ({tag: x.el.tagName, cls: String(x.el.className).slice(0,120), id: x.el.id, left: Math.round(x.rect.left), right: Math.round(x.rect.right), width: Math.round(x.rect.width)})),
    ...
    if (result.overflowElements.length) {
      failures.push(`${pageName} @ ${width}px: horizontal overflow detected in layout elements`);
